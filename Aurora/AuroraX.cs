...
namespace Aurora
{
	...
	public class Engine
	{
		...
		private Action<Response> response;
		...
		#region METHODS
		public void Init(Application app, Request request)
		{
			#region INITIALIZE LOCALS FROM APP/REQUEST AND MISC
			this.request = request;
			this.app = app;
			this.response = app.ResponseCallback;

			int httpStatus = 200;
			ViewResponse viewResponse = null;

			requestType = request.Method.ToLower();
			...
			debugMode = IsAssemblyDebugBuild(Assembly.GetAssembly(GetType()));
			serverError = request.ErrorStackTrace;
			clientCertificate = new X509Certificate2(request.ClientCertificate);
			url = request.Url;
			...
			{
				httpStatus = 503;
				viewResponse = GetErrorViewResponse(request.Error, request.ErrorStackTrace);
			}
		...
		private ViewResponse GetErrorViewResponse(string error, string stackTrace)
		{
			...
			ViewResponse viewResponse = new ViewResponse()
			{
				...
				Content = !string.IsNullOrEmpty(errorView) ? errorView : string.Format("<html><body>{0} : {1}</body></html>", path, error, stackTrace)
			};
		...
		private void RenderResponse(ViewResponse viewResponse, int httpStatus)
		{
			if (string.IsNullOrEmpty(viewResponse.RedirectTo))
			{
				var rep = new Response()
				{
					...
					ContentType = viewResponse.ContentType,
					Body = (viewResponse.Content is string) ? viewResponse.Content.ToString() : null,
					Bytes = (viewResponse.Content is byte[]) ? (byte[])viewResponse.Content : null,
					Status = httpStatus,
					AbandonSession = (currentUser == null) ? true : false
				};
			...
			else
			{
				...
				var rep = new Response()
				{
					Headers = new Dictionary<string, string>() { { "Location", viewResponse.RedirectTo } },
					ContentType = viewResponse.ContentType,
					Status = 301,
					AbandonSession = (currentUser == null) ? true : false
				};
			...
			if (model != null)
			{
				...
				foreach (PropertyInfo p in Model.GetPropertiesWithExclusions(model, true))
				{
					...
					if (skipValidationAttrib == null)
						propertyValue = request.Form[p.Name];
		...
		internal void AddBundle(string name, string[] paths)
		{
			...
			if (paths.Length <= 0)
				throw new Exception("Bundle: {0} contains no valid paths that point to files.");
		...
		internal RouteInfo FindRoute(string path)
		{
			...
			var routeSlice = routeInfos.AsParallel().SelectMany(routeInfo =>
					routeInfo.Aliases, (routeInfo, alias) =>
						new { routeInfo, alias }).OrderByDescending(x => x.alias.Length)
		...
		internal void LogOn(string id, string[] roles, object archeType = null, object special = null)
		{
		...
		{
			return app.GetApplicationCallback(key);
		}

		public void AddApplication(string key, object value)
		{
			app.AddApplicationCallback(key, value);
		}

		public object GetSession(string key)
		{
			return app.GetSessionCallback(key);
		}

		public void AddSession(string key, object value)
		{
			app.AddSessionCallback(key, value);
		}

		public void RemoveSession(string key)
		{
			app.RemoveSessionCallback(key);
		}

		public ViewResult ResponseRedirect(string path, bool fromRedirectOnly)
		{
			if (fromRedirectOnly)
	...
	public abstract class ActionFilterAttribute : Attribute
	{
		...
		public ViewResult Redirect(string alias)
		{
			return engine.ResponseRedirect(alias, false);
		}
	...
	public abstract class BaseController
	{
		...
		public Request Request { get { return engine.request; } }
		public User CurrentUser { get { return engine.currentUser; } }
		public X509Certificate2 ClientCertificate { get { return engine.clientCertificate; } }
		public string Url { get { return engine.url; } }
		...
		internal bool RaiseCheckRoles(CheckRolesHandlerEventArgs checkRolesHandlerEventArgs)
		{
			...
			if (OnCheckRoles != null)
			{
				...
				result = checkRolesHandlerEventArgs.Result;
			}
		...
		protected void LogOn(string id, string[] roles, object archeType = null, object special = null)
		{
			engine.LogOn(id, roles, archeType, special);
		}
		...
		protected ViewResult Redirect(string path)
		{
			return engine.ResponseRedirect(path, false);
		}

		protected ViewResult Redirect(string alias, params string[] parameters)
		{
			return engine.ResponseRedirect(string.Format("{0}/{1}", alias, string.Join("/", parameters)), false);
		}

		protected ViewResult RedirectOnly(string path)
		{
			return engine.ResponseRedirect(path, true);
		}
	...
	public class User
	{
		...
		public object Special { get; internal set; }
	...
	public class ViewResult : IViewResult
	{
		...
		public ViewResult(string location)
		{
		...
		public ViewResponse Render()
		{
			if (headers.ContainsKey("Location"))
				return new ViewResponse() { ContentType = "text/html", Headers = headers };
	...
	public class FileResult : IViewResult
	{
		...
		public FileResult(string name, byte[] data, string contentType)
		{
			...
			if (!string.IsNullOrEmpty(contentType) || Defaults.MimeTypes.ContainsKey(fileExtension))
				contentType = Defaults.MimeTypes[fileExtension];
		...
		public FileResult(Regex allowedFilePattern, string path)
		{
			...
			if (File.Exists(path) && allowedFilePattern.IsMatch(path))
			{
				...
				if (Defaults.MimeTypes.ContainsKey(fileExtension))
				{
					contentType = Defaults.MimeTypes[fileExtension];
		...
		public ViewResponse Render()
		{
			...
			headers["Cache-Control"] = string.Format("public");
